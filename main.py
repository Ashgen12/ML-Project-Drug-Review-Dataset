...
def scrape_website(url):
    ...
    # Perform web scraping logic and extract desired data
...
def main():    
    ...
    ## Perform web scraping using multiprocessing
    num_websites = int(input("Enter the number of websites to scrape: "))
    websites = []
    for i in range(num_websites):
        url = input("Enter the URL of website {}: ".format(i+1))
        websites.append(url)

    pool = multiprocessing.Pool()
    scraped_data = pool.map(scrape_website, websites)   
    
    # Save the output data in CSV format
    output_file = input("Enter the output file name (without extension): ")
    output_df = pd.DataFrame(scraped_data)  # Assuming scraped_data is a list of dictionaries or a pandas DataFrame
